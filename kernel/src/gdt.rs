...
pub const DOUBLE_FAULT_IST_INDEX: u16 = 1; // IST index 1 in IDT
...
pub fn init() {
    ...
    unsafe {
        // 1. Allocate a fresh TSS for this CPU
        let mut tss = alloc::boxed::Box::new(TaskStateSegment::new());
        
        // Allocate Per-CPU Double Fault Stack
        ...
        let tss_ref = alloc::boxed::Box::leak(tss);
        let tss_ptr = tss_ref as *const _ as u64;
        
        // Safety check: The hardware MUST have a 16-byte aligned TSS or it will GPF
        assert!(tss_ptr % 16 == 0, "CPU-GDT: TSS pointer is not 16-byte aligned! ptr={:#x}", tss_ptr);

        // 2. Allocate a fresh GDT for this CPU
        ...
        let gdt_ref = alloc::boxed::Box::leak(alloc::boxed::Box::new(gdt));
        gdt_ref.load();
