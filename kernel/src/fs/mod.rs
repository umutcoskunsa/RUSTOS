...
impl Fat32 {
    ...
    fn from_disk() -> Option<Self> {
        let mut buf = [0u8; 512];
        crate::disk::read_sectors(0, 1, &mut buf).ok()?;
    ...
    fn next_cluster(&self, cluster: u32) -> u32 {
        ...
        let mut buf = [0u8; 512];
        if crate::disk::read_sectors(fat_sector, 1, &mut buf).is_err() {
            return 0x0FFF_FFFF;
        }
    ...
    fn read_chain(&self, start_cluster: u32) -> Vec<u8> {
        ...
        while cluster < 0x0FFF_FFF8 {
            ...
            if crate::disk::read_sectors(sector, secs, &mut data[prev_len..]).is_err() {
                break;
            }
    ...
    fn write_fat_entry(&self, cluster: u32, value: u32) -> Option<()> {
        ...
        let mut buf = [0u8; 512];
        crate::disk::read_sectors(fat_sector, 1, &mut buf).ok()?;
        ...
        crate::disk::write_sectors(fat_sector, 1, &buf).ok()?;
        Some(())
    }
    ...
    fn find_free_cluster(&self) -> Option<u32> {
        ...
        for sec_off in 0..self.fat_sz32 {
            ...
            if crate::disk::read_sectors(sector, 1, &mut buf).is_err() { break; }
    ...
    fn write_chain(&self, start_cluster: u32, data: &[u8]) -> Option<()> {
        ...
        while cluster < 0x0FFF_FFF8 && offset < data.len() {
            ...
            crate::disk::write_sectors(sector, self.sec_per_clus as u8, &sector_buf).ok()?;
            offset  += chunk_len;
            cluster  = self.next_cluster(cluster);
        }
        Some(())
    ...
    fn write_file_inner(&self, filename: &str, data: &[u8]) -> Option<()> {
        ...
        'outer: while cluster < 0x0FFF_FFF8 {
            ...
            for sec_off in 0..self.sec_per_clus {
                ...
                if crate::disk::read_sectors(sector, 1, &mut sbuf).is_err() { break; }
        ...
        let mut sbuf   = [0u8; 512];
        crate::disk::read_sectors(abs_sector, 1, &mut sbuf).ok()?;
        ...
        crate::disk::write_sectors(abs_sector, 1, &sbuf).ok()?;
        Some(())
    }
}
...
/// List all files in the FAT32 root directory.
pub fn list_root() -> Vec<String> {
    Fat32::from_disk().map(|fs| fs.list_root()).unwrap_or_default()
}

/// Read a file from the FAT32 root directory by name.
